...
async function resolveGalleryRoot() {
  // Priority: env var > config.json > prompt
  const isReset = process.argv.includes('--reset');

  if (process.env.GALLERY_ROOT && !isReset) {
    return path.resolve(process.env.GALLERY_ROOT);
  }

  const config = loadConfig();
  if (!isReset && config.galleryRoot) {
    const resolved = path.resolve(config.galleryRoot);
    try {
      await fsp.access(resolved);
      return resolved;
    } catch {
      console.log(`\n⚠️  저장된 경로를 찾을 수 없습니다: ${resolved}`);
    }
  }

  // Interactive prompt
  console.log('\n📁 갤러리 폴더가 설정되지 않았습니다.');
  console.log('   폴더 경로를 입력하세요 (Finder에서 드래그해도 됩니다):');
  const input = await askQuestion('   > ');

  // Clean path (dragged paths may have trailing space or quotes)
  ...
  try {
    ...
    if (!stat.isDirectory()) {
      console.log('❌ 해당 경로는 폴더가 아닙니다. 기본 경로를 사용합니다.');
      return path.join(os.homedir(), 'Pictures');
    }
  } catch {
    console.log('❌ 해당 경로를 찾을 수 없습니다. 기본 경로를 사용합니다.');
    return path.join(os.homedir(), 'Pictures');
  }

  // Save for next time
  const cfg = loadConfig();
  cfg.galleryRoot = resolved;
  saveConfig(cfg);
  console.log(`✅ 저장됨! 다음부터 자동으로 이 폴더를 사용합니다: ${resolved}\n`);
  return resolved;
}
...
async function main() {
  ...
  const GALLERY_ROOT = await resolveGalleryRoot();
